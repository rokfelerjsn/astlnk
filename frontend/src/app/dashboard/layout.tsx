...
const NAVIGATION = [
  ...
  { name: 'Tiket (Kanban)', href: '/dashboard/tickets', icon: KanbanSquare },
  { name: 'Gedung', href: '/dashboard/buildings', icon: Building2 },
  { name: 'Ruangan', href: '/dashboard/rooms', icon: DoorOpen },
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex">
      ...
      {/* Sidebar */}
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-slate-200 transform transition-transform duration-200 ease-in-out lg:static lg:translate-x-0 flex flex-col ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-16 bg-white border-b border-slate-200 flex items-center px-4 lg:hidden">
          ...
          <span className="font-bold text-slate-900 ml-2">AsetLink Admin</span>
        </header>
